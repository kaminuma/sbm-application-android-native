...
- **✨ AI ライフガイド**: カスタマイズ可能なAI分析（期間指定（実装予定）・焦点・詳細度・口調を選択可能）
...
- **パーソナライズ**: 3つのカテゴリで自分好みの分析スタイルを選択
- **永続化対応**: アプリ再起動後も設定が維持される
- **動的プロンプト**: 設定に応じてAIの応答内容が自動調整
- **堅牢性向上**: エラーハンドリング強化とJSON解析の安定化
- **将来対応**: API 経由での AI 提供に向けた設計

### 使い方
1. 分析画面の「✨ AI ライフガイド 🤖」の歯車アイコンをタップ
2. 3つのカテゴリで好みの設定を選択（自動保存）
